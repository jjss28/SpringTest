...
public interface BoardMapper {
	// interface는 클래스와 다르게 메서드가 추상화처럼 되어있다.
	// 추상메서드는 시그니처만 있으며 {실행문이 없다.} 그래서 ; 끝난다.
	
	// 이노테이션으로 간단한 쿼리문은 가능하나 복잡한 쿼리문은 버그가 좀 있다.
	@Select("SELECT * FROM tbl_board WHERE bno > 0") // bno > 0 pk로 선언되어있어서 인덱싱으로 빠른 추출
	public List<BoardVO> getList(); // 추상메서드
	
	// 그래서 xml을 이용하여 sql은 처리를 진행한다.
	// xml을 이용하려면 src/main/resources 폴더 아래쪽에 org 폴더 아래에 mbc 폴더 아래에 mapper 폴더를 만들고
	...
	// 번호가 들어가면 삭제후 몇개가 삭제 됬는지 리턴한다.
	
	public int update(BoardVO board); // xml에서 쿼리 작성
	// 번호가 들어가면 필드 수정후 몇개가 수정됬는지 리턴한다.
